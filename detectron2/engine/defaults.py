...
class DefaultTrainer(SimpleTrainer):
    ...
    def build_hooks(self):
        ...
        if comm.is_main_process():
            ...
            # Put `PeriodicDump` after writers so that can dump all the files,
            # including the files generated by writers
            if cfg.OUTPUT_DUMP.ENABLED:
                period = cfg.OUTPUT_DUMP.PERIOD
                assert period >= cfg.SOLVER.CHECKPOINT_PERIOD, (
                    "Dump period ({}) must large than checkpoint period ({}).".format(
                        period, cfg.SOLVER.CHECKPOINT_PERIOD
                    )
                )
                platforms = cfg.OUTPUT_DUMP.PLATFORMS
                buckets = cfg.OUTPUT_DUMP.BUCKETS
                ret.append(hooks.PeriodicDump(cfg.OUTPUT_DIR, period, platforms, buckets))
